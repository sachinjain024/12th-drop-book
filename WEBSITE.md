...
Technical notes for [12thdrop.in](https://12thdrop.in) — the single-page website for the book **12th Drop · बारहवीं ड्रॉप**. For the book itself, see the [README](README.md).
...
- **Publish free:** drag the whole folder into [Netlify Drop](https://app.netlify.com/drop), or push it to a GitHub repo and enable **GitHub Pages**. Either gives you a live URL in minutes. (This repo deploys via GitHub Pages with the custom domain in `CNAME`.)
...
3. **Reviews** — review cards (each with a Hindi + English quote); placeholder "coming soon" cards fill any empty slots.
...
Everything editable is clearly commented in `index.html`. See `CLAUDE.md` for exact instructions. The most common edits:

- **Add a real review** — convert a `placeholder` card into a real one, using the first review card as the template.
- **Change the Kindle link** — the URL `https://www.amazon.in/dp/B0H7CKSGB2` appears in the nav, hero, BUY section, and footer.
